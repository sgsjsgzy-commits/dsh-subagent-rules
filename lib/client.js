...
`;
    ...
    function SubagentRulesSelector(props) {
      ...
      const { inputActions, input, loadModels } = props;
      ...
      const canInsert = !!(inputActions?.setDraft && input);

      const insert = () => {
        if (!provider || !model || !canInsert) return;
        const found = findModel(catalog, provider, model);
        const group = found?.group;
        const m = found?.model;
        const efforts = effortListFor(m);
        const effortObj = efforts.find((e) => e.id === effort);
      ...
      const buttonStyle = {
        ...
        cursor: "pointer",
        background: inserted ? "var(--dsw-alias-interactive-bg-hover)" : "transparent",
        border: inserted ? "1px solid var(--dsw-alias-border-l3)" : "none",
        borderRadius: 24,
        display: "inline-flex",
        alignItems: "center",
        gap: 4,
      ...
      const triggerLabel = provider && model
        ? `子代理 · ${model} · ${effort || "默认"}`
        : "子代理";
      ...
      return react.createElement(
        ...
        open &&
          react.createElement(
            "div",
            { role: "menu", "aria-label": "子代理分发设置", "aria-busy": loading, style: menuStyle },
            step === "models" &&
              react.createElement(
                react.Fragment,
                null,
                react.createElement("div", { style: sectionLabelStyle }, "选择子代理模型（全部提供商）"),
                loading && react.createElement("div", { role: "status", style: { padding: "8px", fontSize: 12 } }, "加载模型中…"),
                error &&
                  react.createElement(
                    react.Fragment,
                    null,
                    react.createElement("div", { role: "alert", style: { color: "var(--dsw-alias-state-error-primary)", padding: "8px", fontSize: 12 } }, error),
                    react.createElement(
                      "button",
                      { type: "button", style: { ...optionStyle(false), justifyContent: "center", fontWeight: 600 }, onClick: load },
                      "重试"
                    )
                  ),
                !loading && !error && (!catalog || !(catalog.groups || []).length) &&
                  react.createElement("div", { style: { color: "var(--dsw-alias-label-tertiary)", padding: "8px", fontSize: 12 } }, "没有可用的模型"),
            ...
            step === "effort" &&
              react.createElement(
                ...
                react.createElement(
                  ...
                  {
                    type: "button",
                    className: inserted ? "dsr-insert-btn dsr-inserted" : "dsr-insert-btn",
                    style: { ...insertBtnStyle, ...(canInsert ? {} : { opacity: 0.5, cursor: "not-allowed" }) },
                    disabled: !canInsert,
                    onClick: insert,
                  },
                  inserted ? "✓ 已插入（可再次插入）" : "插入子代理提示词"
                )
              )
          )
      );
