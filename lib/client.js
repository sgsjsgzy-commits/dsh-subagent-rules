...
`;
    ...
    function SubagentRulesSelector(props) {
      ...
      // 0.2.1: refresh the session's stored default every time the menu opens,
      // so the button label and the popover always match the persisted state
      // (and don't linger after a set/clear elsewhere).
      useEffect(() => {
        if (!open || typeof loadDefault !== "function") return;
        let cancelled = false;
      ...
      // Label stays terse and unambiguous — it only signals whether a session
      // default exists. Concrete model/effort details live inside the popover
      // ("会话默认派发" section), so the button never shows a truncated or
      // surprising model id/effort string on its own.
