...
`;
    ...
    function SubagentRulesSelector(props) {
      ...
      // 0.2.1: load the session's stored default on mount AND refresh whenever
      // the menu opens, so the button label always matches the persisted state.
      // A failed read (__unreadable) keeps the last value — never flips to
      // 'unset' while the host still has the default and keeps routing.
      useEffect(() => {
        ...
        loadDefault().then((value) => {
          if (cancelled) return;
          if (value && value.__unreadable) return;
          setDefaultCfg(value && (value.provider || value.model) ? value : null);
        }).catch(() => {
          // read error: keep whatever was last shown
        }).finally(() => {
      ...
      // Label shows the concrete default so the user knows the active route at
      // a glance: "子代理默认 · <model> · <effort>". It only flips back to the
      // bare "子代理默认" after a confirmed clear (not after a failed read).
      const triggerLabel = defaultCfg && defaultCfg.model
        ? `子代理默认 · ${defaultCfg.model} · ${defaultCfg.effort || "父级强度"}`
        : "子代理默认";
      ...
      ctx.slots.inject("conversation.input.right", () =>
        ctx.slots.register({
          ...
          inject: (sessionId) => ({
            ...
            // A read must distinguish "no default set" ({} -> shows unset) from
            // "could not read" ({ __unreadable: true } -> keeps the last shown
            // state, so the UI never flips to 'unset' while the host still has
            // the default and keeps routing).
            loadDefault: async () => {
              if (typeof sessionId !== "string" || sessionId === "") return { __unreadable: true };
              try {
                const response = await fetch(`${DEFAULT_ROUTE}?sessionId=${encodeURIComponent(sessionId)}`);
                if (!response.ok) return { __unreadable: true };
