...
export function apply(ctx, config) {
  ...
  // strip. To keep anchored's FIRST request Minimal-exact (its whole
  // mechanism is a clean request #1), the injection is gated on PROMOTION:
  // it lands only after the session records its first durable tool/call or
  // assistant/message — the same gate the anchored instruction-hint uses.
  // Once per session (in-process set); the rules also live in the tool
  // descriptions (subagent_flash, dev_tool_search index), so they stay
  // reachable even after compaction.
  if (cfg.injectRules) {
