      ...
      ctx.tools.register({
        ...
        async execute(args, exec) {
          ...
          if (args.reasoningEffort && args.reasoningEffort !== 'auto') {
            const level = String(args.reasoningEffort).trim().toLowerCase()
            if (!EFFORT_VALUES.includes(level)) {
              throw new Error(`invalid reasoningEffort "${args.reasoningEffort}": use max | high | medium | low | minimal | auto`)
            }
            prompt = `${prompt}\n\n[subagent effort] First call effort_set ${level} in your session, then start working.`
          }
          const result = await ctx.subagents.startContinuable({
            provider: cfg.provider,
            label: args.description,
