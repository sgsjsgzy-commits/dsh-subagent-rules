 ...
 *   3. RULES INJECTION: a `subagent-rules` system-prompt section tells every
 *      model which tool to use, what the effort default is, and how to ask a
 *      child for a non-default effort — no per-session coaching needed.
 *
...
/** The tools registry, the subagents service, and the system-prompt service. */
...
export function apply(ctx, config) {
  ...
  //    selection; subagents never do — this is the gap being closed). ──
  ...
  ctx.effect(() => ctx.tools.register({
    ...
    async execute(args, exec) {
      ...
      if (value.length === 0) return 'usage: effort_set <max|high|medium|low|minimal|auto>'
      if (value === 'auto') overrides.delete(session.id)
      else overrides.set(session.id, value)
      const current = overrides.get(session.id) ?? defaultEffort
      return `effort=${current} (session ${String(session.id).slice(0, 8)}) — next request applies`
    },
  ...
  // ── rules section: every conversation reads the dispatch rules ──────────
  if (cfg.injectRules) {
    ctx.effect(() => ctx.systemPrompt.section({
      name: 'subagent-rules',
      order: 500,
      text: rulesText(cfg),
    }), 'dsh-subagent-rules: rules section')
  }
}
